...
const PLAYER = {
  ...
  // Sound starts ON: the game effects are core to how it feels, and a child opening
  // it for the first time shouldn't have to find Profile → Sound effects first to
  // notice anything plays at all. Turn it off there, or from the Tweaks panel.
...
const TODAY_TASKS = [
  // `route` = where the 'Go' button sends the child to actually do the task.
  { id: 't1', scope: 'daily', icon: 'footprints',  title: 'Finish a phone-free walk',  reward: 100, done: true,  route: 'safety' },
  { id: 't2', scope: 'daily', icon: 'shield-check', title: 'Reach your safe-walk goal', reward: 50,  done: false, route: 'streak' },
  { id: 't3', scope: 'daily', icon: 'heart-handshake', title: 'Say hi to your buddy',   reward: 20,  done: false, route: 'myhouse' },
];
...
/* `img` names a finished medallion in /assets/badges (art already carries the
   plate, ring, ribbon and central icon). When present, Badges.jsx renders the
   image and skips the drawn rosette + lucide `icon`; `icon` stays as the
...
// phone, so it carries its own sound toggle (Profile → General → Sound effects).
