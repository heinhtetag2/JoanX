  ...
  },
  {
    ...
    items: [
      ...
      { id: 'F-18', title: 'Collection House (≥ 3 Rooms)', mode: 'Smart', status: 'done',
        screens: ['MyHouse.jsx'],
        note: '3 basic rooms — Green / Town / Dream — each its own environment and decor set, plus a condition-locked seasonal room (Winter event) showing the expansion path. Rooms are never sold: Green is the home room, the rest are earned in sequence by safe behaviour (ROOM_UNLOCKS, retuned by setRoomUnlocks()).' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'A-6', title: 'Collection House ≥ 3 Rooms, free placement', mode: 'Smart', status: 'done',
        ...
        note: 'Buddies AND items are placed freely per room: tap a buddy to move it in or out (up to the room\'s slots), tap an item to place it. Each room keeps its own placement. Rooms are EARNED, never bought — Green is the home room; Town opens on a 5-day accident-free streak and Dream on 25h of cumulative safe walking, and only after Town (the ladder is sequential). A locked room shows the goal and the live progress toward it, so it reads as a reason to walk rather than a closed door. Every bar, metric and ordering is a server setting — setRoomUnlocks() validates the payload and falls back to the launch defaults, and can never lock the home room.' },
