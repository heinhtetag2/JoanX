...
function CharVariant({ ctx, variant }) {
  ...
  const setBtn = () => (
    <Button key="set" variant={xpAddStyle === 'cta' ? 'outline' : 'primary'} size="lg" fullWidth style={xpAddStyle === 'cta' ? { marginTop: 2, borderColor: tint(THEME.fg3, .45) } : { marginTop: 2, background: appAccent, boxShadow: 'none' }} onClick={() => { ctx.setBuddy(orig.id, { color, stage, level, species: orig.species, name: orig.name }); ctx.nav('home'); }}>{L('Set as my buddy')}</Button>
  );
