...
const HANDOFF = {
  ...
  p_groups:       { status: 'done', note: 'New. Groups list — every circle this guardian belongs to, role badge, children in each, and waiting join requests.' },
  p_group_create: { status: 'done', note: 'New. Create a group — name it, pick which children it covers; creator becomes admin and their main guardian.' },
  p_group_detail: { status: 'done', note: 'New. One group — pending requests (admin only, accept/reject), guardians, children with their main guardian, activity log, leave group.' },
  p_group_invite: { status: 'done', note: 'New. Invite by QR — durable code an admin can rotate, share as an image; replaces the 48h single-use invite link.' },
  p_group_join:   { status: 'done', note: 'New. Join by scanning — QR → group preview → request → waiting for approval → accepted / declined.' },
