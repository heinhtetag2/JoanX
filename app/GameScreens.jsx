...
function Collection({ ctx }) {
  ...
  return (
    <div className="no-sb" style={{ position: 'absolute', inset: 0, overflowY: 'auto', paddingTop: 102, paddingBottom: 110, background: THEME.surface2 }}>
      <ScreenHeader title={L('Collection House')} onBack={() => ctx.back()} right={<div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Icon name="gem" size={15} color={THEME.gold} stroke={2.3} /><span className="game-font" style={{ fontSize: 14, fontWeight: 700 }}>{owned.length}/8</span></div>} />
    ...
    <div className="no-sb" style={{ position: 'absolute', inset: 0, overflowY: 'auto', paddingTop: 102, paddingBottom: 110, background: THEME.surface2 }}>
      ...
      <div style={{ padding: '0 16px' }}>
        ...
        <div style={{ borderRadius: 24, padding: '18px', background: `linear-gradient(165deg, ${shade(color, 74)}, #fff 75%)`, boxShadow: THEME.shadowCard, textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="game-font" style={{ fontSize: 25, fontWeight: 700, marginTop: 4 }}>{orig.name}</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 12 }}>
            ...
            <span className="game-font" style={{ fontSize: 12, fontWeight: 700 }}>{orig.xp}/{orig.xpMax}</span>
          </div>
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 18, padding: 16, boxShadow: THEME.shadowCard, marginBottom: 14 }}>
          ...
          {traits.map(t => (
            <div key={t.k} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              ...
              <span className="game-font" style={{ fontSize: 12, fontWeight: 700, width: 24, textAlign: 'right' }}>{orig.traits[t.k] || 50}</span>
            </div>
          ))}
        </div>
      ...
      {evolving && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(20,24,38,.4)' }} className="jx-fade">
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div className="game-font" style={{ color: '#fff', fontSize: 15, fontWeight: 700, marginTop: 6, opacity: .92 }}>{L('Stage')} {stage} · {L('Level')} {level}</div>
          </div>
        </div>
      )}
    </div>
