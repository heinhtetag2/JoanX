...
function StatCard({ icon, color, bg, value, label, big }) {
  return (
    <div style={{ flex: 1, background: '#fff', borderRadius: 18, padding: 14, boxShadow: THEME.shadowCard }}>
      ...
      <div className="game-font" style={{ fontSize: big ? 26 : 22, fontWeight: 400, color: THEME.fg1, lineHeight: 1 }}>{value}</div>
    ...
    <div style={{ position: 'absolute', inset: 0, background: '#fff', display: 'flex', flexDirection: 'column', paddingTop: 50 }}>
      ...
      {step === 0 && (
        <Hero>
          ...
          <h1 className="game-font" style={{ fontSize: 27, fontWeight: 400, margin: '10px 0 8px' }}>{L('Walk safe, grow your buddy')}</h1>
      ...
      {step === 1 && (
        <Hero>
          <h1 className="game-font" style={{ fontSize: 24, fontWeight: 400, margin: '4px 0 6px', whiteSpace: 'nowrap' }}>{L('Pick a mode')}</h1>
      ...
      {step === 2 && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 24px 0' }} className="no-sb">
          <h1 className="game-font" style={{ fontSize: 24, fontWeight: 400, margin: '4px 0 6px', whiteSpace: 'nowrap' }}>{L('Allow access')}</h1>
      ...
      <div style={{ padding: '8px 18px 4px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <button onClick={() => ctx.nav('profile')} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>
          ...
          <div>
            ...
            <div className="game-font" style={{ fontSize: 21, fontWeight: 400, color: THEME.fg1 }}>{PLAYER.name}</div>
          </div>
        </button>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button onClick={() => ctx.nav('shop')} style={{ display: 'flex', alignItems: 'center', gap: 5, background: '#fff', padding: '7px 12px', borderRadius: 999, boxShadow: THEME.shadowCard, border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
            ...
            <span className="game-font" style={{ fontSize: 15, fontWeight: 400 }}>{PLAYER.coins}</span>
          </button>
      ...
      <div style={{ padding: '8px 16px 0' }}>
        ...
        <div onClick={() => ctx.nav('character', { id: c.id })} style={{ position: 'relative', borderRadius: 24, padding: '18px 18px 20px', marginBottom: 14, cursor: 'pointer', overflow: 'hidden', background: `linear-gradient(160deg, ${shade(c.color, 78)} 0%, ${THEME.surface} 70%)`, boxShadow: THEME.shadowCard }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              ...
              <div className="game-font" style={{ fontSize: 24, fontWeight: 400, marginTop: 8 }}>{c.name}</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            ...
            <span className="game-font" style={{ fontSize: 12, fontWeight: 400, color: THEME.fg1 }}>{c.xp}/{c.xpMax}</span>
          </div>
        ...
        <div style={{ background: '#fff', borderRadius: 18, padding: 16, marginBottom: 16, boxShadow: THEME.shadowCard }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            ...
            <span className="game-font" style={{ fontSize: 13, fontWeight: 400, color: THEME.primary }}>{PLAYER.safeMinutesToday}/{PLAYER.safeWalkGoal} {L('min')}</span>
          </div>
  ...
  return (
    <div className="no-sb" style={{ position: 'absolute', inset: 0, overflowY: 'auto', paddingTop: 56, paddingBottom: 110, background: THEME.surface2 }}>
      <div style={{ padding: '0 18px' }}>
        <h1 className="game-font" style={{ fontSize: 25, fontWeight: 400, margin: '0 0 4px' }}>{L('Safety')}</h1>
        ...
        <div style={{ background: '#fff', borderRadius: 22, padding: '26px 18px', textAlign: 'center', boxShadow: THEME.shadowCard, marginBottom: 14 }}>
          ...
          <div className="game-font" style={{ fontSize: 20, fontWeight: 400 }}>{lite ? L('Lite mode active') : L('Active & protected')}</div>
    ...
    <div className="no-sb" style={{ position: 'absolute', inset: 0, overflowY: 'auto', paddingTop: 102, paddingBottom: 110, background: THEME.surface2 }}>
      ...
      <div style={{ padding: '0 16px' }}>
        ...
        <div style={{ background: `linear-gradient(165deg, ${shade(c.color, 76)}, #fff 78%)`, borderRadius: 22, padding: '20px 18px', boxShadow: THEME.shadowCard, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <div className="game-font" style={{ fontSize: 24, fontWeight: 400 }}>{PLAYER.name}</div>
