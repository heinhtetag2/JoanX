  ...
  if (phase === 'matching') {
    return (
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(170deg,#2b2926,#2b5782)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
        ...
        <div className="game-font" style={{ color: '#fff', fontSize: 21, fontWeight: 400 }}>{L('Finding opponent…')}</div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 24px', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', justifyContent: 'space-around' }}>
            <div style={{ textAlign: 'center', opacity: result && !won ? .4 : 1, transition: 'opacity .4s' }}>
              ...
              <div className="game-font" style={{ color: '#fff', fontSize: 16, fontWeight: 400, marginTop: 4 }}>{sel.name}</div>
              <div style={{ color: 'rgba(255,255,255,.7)', fontSize: 12 }}>Lv {sel.level} · PWR {power(sel)}</div>
            </div>
            <div className="game-font" style={{ color: THEME.gold, fontSize: 26, fontWeight: 400 }}>VS</div>
            <div style={{ textAlign: 'center', opacity: result && won ? .4 : 1, transition: 'opacity .4s' }}>
              <div style={{ transform: 'scaleX(-1)' }}><Mascot species={opp.species} stage={2} color={opp.color} size={120} /></div>
              <div className="game-font" style={{ color: '#fff', fontSize: 16, fontWeight: 400, marginTop: 4 }}>{opp.name}</div>
          ...
          {result && (
            <div className="jx-pop" style={{ marginTop: 30, textAlign: 'center' }}>
              <div className="game-font" style={{ fontSize: 36, fontWeight: 400, color: won ? THEME.gold : '#fff' }}>{won ? L('Victory!') : L('So close!')}</div>
              {won && <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: THEME.goldLight, color: '#9e7300', padding: '8px 16px', borderRadius: 999, fontWeight: 500, fontSize: 15, marginTop: 12 }} className="game-font"><Icon name="star" size={16} color={THEME.gold} fill={THEME.gold} stroke={2} /> +120 points · +2 coins</div>}
    ...
    <div className="no-sb" style={{ position: 'absolute', inset: 0, overflowY: 'auto', paddingTop: 102, paddingBottom: 110, background: THEME.surface2 }}>
      ...
      <div style={{ padding: '0 16px' }}>
        ...
        <div style={{ borderRadius: 24, padding: 18, background: `linear-gradient(165deg, ${shade(sel.color, 74)}, #fff 75%)`, boxShadow: THEME.shadowCard, textAlign: 'center', marginBottom: 16 }}>
          ...
          <div className="game-font" style={{ fontSize: 22, fontWeight: 400 }}>{sel.name}</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 14, marginTop: 8 }}>
            {[['shield', sel.traits.guard], ['gauge', sel.traits.speed], ['heart', sel.traits.heart]].map(([ic, v], i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <span className="game-font" style={{ fontSize: 13, fontWeight: 400 }}>{v}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="no-sb" style={{ position: 'absolute', inset: 0, overflowY: 'auto', paddingTop: 56, paddingBottom: 110, background: THEME.surface2 }}>
      <div style={{ padding: '0 18px' }}>
        <h1 className="game-font" style={{ fontSize: 25, fontWeight: 400, margin: '0 0 16px' }}>{L('Rewards')}</h1>
        ...
        <div style={{ borderRadius: 22, padding: 18, background: 'linear-gradient(160deg,#fff2d1,#fff 80%)', boxShadow: THEME.shadowCard, marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            ...
            <div>
              <div className="game-font" style={{ fontSize: 22, fontWeight: 400 }}>{PLAYER.streak}{L('-day streak')}</div>
        ...
        {pop && (
          <div className="jx-fade" style={{ position: 'absolute', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(43,41,38,.34)' }}>
            ...
            <div className="jx-pop" style={{ width: 244, background: '#fff', borderRadius: 26, padding: '24px 20px', textAlign: 'center', boxShadow: THEME.shadowXl }}>
              ...
              <div className="game-font" style={{ fontSize: 21, fontWeight: 400 }}>{L('Daily reward claimed!')}</div>
              <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 12 }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: THEME.goldLight, color: '#9e7300', padding: '8px 14px', borderRadius: 999, fontWeight: 500, fontSize: 15 }} className="game-font"><Icon name="star" size={16} color={THEME.gold} fill={THEME.gold} stroke={2} />+100</span>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: THEME.goldLight, color: '#9e7300', padding: '8px 14px', borderRadius: 999, fontWeight: 500, fontSize: 15 }} className="game-font"><Icon name="coins" size={16} color={THEME.gold} stroke={2.2} />+5</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div style={{ padding: '0 16px' }}>
        ...
        <div style={{ borderRadius: 22, padding: '20px 18px', background: 'linear-gradient(160deg,#fff2d1,#fff 78%)', boxShadow: THEME.shadowCard, marginBottom: 14, textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            ...
            <span className="game-font" style={{ fontSize: 40, fontWeight: 400, lineHeight: 1 }}>{coins}</span>
          </div>
