...
| F-32.1 | Add-friend methods (QR · code · nickname) + approval | ✅ | `AddFriends` `data.jsx` | All three methods route through one method switcher driven by the `FRIEND_METHODS` registry, so a method can be disabled, reordered or added (`enabled` flag) without touching the screen. Nicknames are **not unique**, so search results show each match's system **friend code** to disambiguate (`DISCOVERABLE_USERS` seeds two same-nickname users to prove it). A request becomes a friendship only on the recipient's accept — `FRIEND_POLICY = { requiresApproval, approver: 'recipient' }`, one knob to move to auto/mutual/parent approval |
| F-33 | No friend/interaction limits · scales | ✅ | `Friends` `data.jsx` | Friends, requests, likes and gifts are **uncapped by design**, written down as explicit policy (`FRIEND_LIMITS = { friends: null, requests: null, likes: null, gifts: null }`) so no later change silently adds a ceiling. Scale is handled at the render layer, not by capping: the default friends list **windows in a page at a time** via an `IntersectionObserver` sentinel, so a very long list keeps only what's scrolled-to in the DOM, and every interaction (`react`, add-by-code) is O(1) on its own record. Likes stay one-reaction-per-friend (a per-friend rule, not a quota on how many friends you may like) |
...
| A-13.1 | Up to 5 managed children per guardian | ✅ | `ParentChildren` `ParentAddChild` `data.jsx` | One guardian manages up to `MAX_CHILDREN` children — "managed" = has authority over, related or not. One configurable knob (default **5**) so a family/school plan can raise it in a single place. Enforced at the registration mutation (`addChild` slices to remaining capacity), not just the UI, so no path pushes past it; the add affordances hide/disable at the cap and the header shows `n/5`. Each child already carries its own profile, collection, safe-walking data and reports, and the parent switches between them across `ParentReports` / `ParentDetail` / `ParentSettings` — this row only added the ceiling |
